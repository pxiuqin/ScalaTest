...
def subdirs(dir: File): Iterator[File] = {
  val childeren = dir.listFiles.filter(_.isDirectory)
  childeren.toIterator ++ childeren.toIterator.flatMap(subdirs(_))
}

//访问所有子目录
for(d<-subdirs(new File("test.txt"))) println("dir")
